...
for regionName = {'CPM_NW','CPM_NE','CPM_S'}% {'SCO','WAL','EUK'}% 
    ...
    ylabel('');
    ax.YTickLabel = [];
    % savePlot([savePath,filesep,'test'],'targetSize','dc','needreply','Y');% 'XYWH',[150,0,750,320],'needreply','N');
    figure(3)
...
A = grpstats(T,{'RD0','ensNo'},{'median',@(x)quantile(x,.75),@(x)quantile(x,.25)});
RD0 = A.RD0;
DAT0 = A.median_DAT0;
ensNo = A.ensNo;
...
summary.spatialCoverage_range = squeeze(nansum(squeeze(nanmedian(Ra,3))>5,[1,2])*2.2*2.2);
summary.totalVolume_range = squeeze(nansum(squeeze(nanmedian(Ra,3)),[1,2])*10^-3/3600*2200*2200);
summary.peakIntensity_range = squeeze(nanmax(squeeze(nanmedian(Ra,3)),[],[1,2]));
% Ra = squeeze(nanmedian(nanmean(Ra,3),4));
Ra = squeeze(nanmedian(nanmedian(Ra,3),4));
...
% plot(ax.XLim,20*ones(2,1),'k:','Color',[0.1 0.1 0.1],'linewidth',0.5);
ax.YTick = [-50,-20,0,20,50]+1.1;
ax.YTickLabels = ax.YTick-1.1;
ax.XTick = [-50,-20,0,20,50]+1.1;
ax.XTickLabels = ax.XTick-1.1;
hold off;
box on
axis('square')
