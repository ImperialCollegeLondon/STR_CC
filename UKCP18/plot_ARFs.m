...
for regionName = {'EUK','WAL','SCO'}%{'WAL'}% 
   ...
   % [Dat_past,Dat_future,arf] = getData(dataSourcePath,regionName,ensNo);
    [Dat_past,Dat_future,arf] = getData(dataSourcePath,regionName,getEnsNos());

    subplot 121
    hold on;
    [hpas,hfut] = deal(handle(0));
    for duri = [1]%1:length(durVec)
        dur = durVec(duri);
        ...
        val = Dat_future.arf(Dat_future.dur==dur,:);
        hfut(duri) = plot([1,arf.area]*2.2*2.2,[1,nanmean(val,1)],...
            '--.','color',[colfut,0.5],'markerfacecolor',[colfut],'markersize',10);
    end
    formatSubP1(hpas,hfut,durVec([1]))
    hold off
    ...
    for duri = [1] %1:length(durVec)
        dur = durVec(duri);
        col = getColorMat(duri,'fut');
        hpas(duri) = plot([arf.area]*2.2*2.2,...
            nanmean(Dat_future.arf(Dat_future.dur==dur,:),1)./nanmean(Dat_past.arf(Dat_past.dur==dur,:),1),...
            'o-','color',[col,0.5],'markerfacecolor',[col]);
    end
    formatSubP2(hpas,hfut,durVec([1]))
    hold off
    
    figPath = 'C:\Users\Yuting Chen\Dropbox (Personal)\Data_PP\Fig_UKCP\ARF';
    fileName = sprintf('%s%sARF_NERC_JJA_%s',figPath,filesep,regionName);
...
function formatSubP2(hpas,hfut,durVec)
ylim([0.85,1.15])
grid minor
box on;
ax = gca;
...
function formatSubP1(hpas,hfut,durVec)
ylim([0.5,1])
grid minor
box on;
ax = gca;
legend(flip([hpas,hfut]),flip({strcat(string(durVec'),'h-past'),...
