...
%         
        ...
        War1 = arrayfun(@(x)cell2mat(cellfun(@(y)y.WAR,x,'UniformOutput', false)),...
            CPM.dataplot{thisseason},'UniformOutput', false);
        ...
        % do it for radar
        axes(ha(tsp+4));
        [X,Y,GMAP_rad] = plotJointDist([RAD.dataplot{thisseason}.WAR,...
    ...
    filename = [filePath,filesep,'DUR_mW_rad_cpm_hist3_Allvalue_',Name{thisstation}];
    ...
    filePath = 'C:\Users\Yuting Chen\Dropbox (Personal)\Data_PP\Fig_UKCP\TemporalAnalysis\';
    filename = [filePath,filesep,'DUR_mW_rad_cpm_hist3_',Name{thisstation}];
